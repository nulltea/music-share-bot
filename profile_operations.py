...
def add_profile_actions(self):
	self.action_dictionary["profile"]["firstname"] = partial(
		self.edit_action,
		hint="Sorry, what should I call you then?",
		property="first_name"
	);
	self.action_dictionary["profile"]["lastname"] = partial(
		self.edit_action,
		hint="Sorry, what should I call you then?",
		property="last_name"
	);
	self.action_dictionary["profile"]["bio"] = partial(
		self.edit_action,
		hint="Great can't wait to read more about you, so?",
		property="bio"
	);
...
def registration(self, message):
	self.bot.send_message(
		message.chat.id,
		"Perfect. First, can you check this information about yourself that I've gathered from your profile?"
	);
	new_user = BotUser(
	...
	self.bot.send_photo(message.chat.id, new_user.profile_picture, new_user.generate_message,
						reply_markup=new_user.edit_menu, parse_mode='Markdown');


def save_profile(self, call, profile):
	menu = telebot.types.InlineKeyboardMarkup();
	menu.row(
		telebot.types.InlineKeyboardButton(text="Yes", callback_data=f"profile/save/yes/{profile.pk}"),
...
def save_yes(self, call, profile):
	...
	self.bot.send_message(
		...
		"Now you can add your music tracks to shared collection!",
